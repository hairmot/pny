module.exports = function createDirectionMap(state) {
   var map = state.data.map((a, b) => {
        return {
            "directions": [
                { direction: 'north', cell: a.indexOf('north') === -1 ? b - state.size[1] : false },
                { direction: 'south', cell: b < (state.size[0] * (state.size[1] -1)) ? state.data[b + state.size[1]].indexOf('north') === -1 ? b + state.size[1] : false : false },
